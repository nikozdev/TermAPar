...
void ArgStream::append(const string &arg) { args.push_back(arg); }
	...
	for(char &c: arg)
	{
		...
		if(options.count(name) > 0)
		{
			...
			else
			{
				if(arg.size() > 1)
				{
					cerr << "Error: missing argument for '" << c << "' in -"
						 << arg << ".\n";
				}
...
{
	ArgStream stream;
	for(string &arg: args) { stream.append(arg); }
	parse(stream);
}
...
ArgParser::~ArgParser()
{
	...
	for(auto element: commands) { unique_cmd_parsers.insert(element.second); }
