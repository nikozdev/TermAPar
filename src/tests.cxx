...
void test_flag_not_found()
{
	nTermArgs::ArgParser parser;
	parser.flag("foo f");
	parser.parse(std::vector<std::string>({ "abc", "def" }));
	assert(parser.found("foo") == false);
	assert(parser.count("foo") == 0);
	printf(".");
}
...
void test_flag_long()
{
	nTermArgs::ArgParser parser;
	parser.flag("foo f");
	parser.parse(std::vector<std::string>({ "--foo" }));
...
void test_flag_short()
{
	nTermArgs::ArgParser parser;
	parser.flag("foo f");
	parser.parse(std::vector<std::string>({ "-f" }));
	assert(parser.found("foo") == true);
	assert(parser.count("foo") == 1);
	printf(".");
}
...
void test_flag_condensed()
{
	nTermArgs::ArgParser parser;
	parser.flag("foo f");
	parser.parse(std::vector<std::string>({ "-fff" }));
	assert(parser.found("foo") == true);
...
void test_flag_multi()
{
	nTermArgs::ArgParser parser;
	parser.flag("foo f");
	parser.parse(std::vector<std::string>({ "-fff", "--foo", "-f" }));
	assert(parser.found("foo") == true);
...
void test_option_long()
{
	nTermArgs::ArgParser parser;
	parser.option("foo f", "default");
	parser.parse(std::vector<std::string>({ "--foo", "bar" }));
	assert(parser.found("foo") == true);
	assert(parser.count("foo") == 1);
...
void test_option_short()
{
	nTermArgs::ArgParser parser;
	parser.option("foo f", "default");
	parser.parse(std::vector<std::string>({ "-f", "bar" }));
	assert(parser.found("foo") == true);
	assert(parser.count("foo") == 1);
	assert(parser.value("foo") == "bar");
	printf(".");
...
void test_option_condensed()
{
	nTermArgs::ArgParser parser;
	parser.option("foo f", "default");
	parser.parse(std::vector<std::string>({ "-ff", "bar", "baz" }));
	assert(parser.found("foo") == true);
...
void test_option_multi()
{
	nTermArgs::ArgParser parser;
	parser.option("foo f", "default");
	parser.parse(std::vector<std::string>({ "-ff", "bar", "baz", "--foo", "bam" }
	));
	assert(parser.found("foo") == true);
	assert(parser.count("foo") == 3);
...
{
	nTermArgs::ArgParser parser;
	parser.parse(std::vector<std::string>({ "foo", "--", "--bar", "--baz" }));
...
{
	nTermArgs::ArgParser  parser;
	nTermArgs::ArgParser &cmd_parser = parser.command("boo");
	cmd_parser.flag("foo");
	cmd_parser.option("bar", "default");
	parser.parse(std::vector<std::string>(
		{ "boo", "abc", "def", "--foo", "--bar", "baz" }
	));
